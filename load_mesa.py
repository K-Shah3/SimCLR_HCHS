...
if __name__ == "__main__":
    # load_data_and_active_labels(path_to_actigraphy_data, path_to_pickled_mesa_data)

    with open(path_to_data_label_dict, 'rb') as f:
        data_label_dict = pickle.load(f)

    # sample_key = list(data_label_dict.keys())[0]
    # print(data_label_dict[sample_key])

    get_fixed_test_train_split_and_pickle(data_label_dict, path_to_pickled_mesa_data)
    get_fixed_test_train_split_reduced_and_pickle(data_label_dict, path_to_pickled_mesa_data)