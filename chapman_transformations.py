...
def noise_transform_vectorized(X, sigma=10):
    """
    Adding random Gaussian noise with mean 0 and sigma of 10, according to Han et al. 
    """
    noise = np.random.normal(loc=0, scale=sigma, size=X.shape)
    return X + noise

def scaling_transform_vectorized(X, sigma=10):
    """
